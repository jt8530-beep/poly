...
and `os.getenv()` would silently fall back to defaults — same bug as
lp_market_maker (2026-05-26 PLTR loss postmortem).
"""
...
def _load_dotenv() -> int:
    """
    Read `<package>/../.env` (e.g. `overround_arb/.env`) and inject any
    ...
    try:
        ...
        logging.getLogger("oa.config").warning(".env load failed: %s", e)
    return n


# CRITICAL: this MUST run before the @dataclass class bodies below are
# evaluated, because dataclass field defaults like `_env_s("OA_X", "")` are
...
def load() -> Config:
    ...
    if _LOADED_DOTENV_COUNT:
        logging.getLogger("oa.config").info(
