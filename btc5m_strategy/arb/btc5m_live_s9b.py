...
Extra rule vs S9:
  6. Skip signals where edge_bps > BTC5M_MAX_EDGE_BPS (default 1800)

Controlled by:
  BTC5M_MAX_EDGE_BPS=1800  (set to 0 or negative to disable)
"""
...
def run_once(db: sqlite3.Connection, poly: PolyClient, price: BinancePrice, tg: Notifier | None = None) -> dict:
    ...
    max_edge = _env_i("BTC5M_MAX_EDGE_BPS", 1800)  # S9B: edge cap
    ...
    if int(best["edge_bps"]) < min_edge:
        ...
        if notional < _env_f("BTC5M_MIN_NOTIONAL_USD", 5.0):
            ...
            if tg:
                tg.send_html(
                    "<b>BTC5M S9B Sydney candidate</b>\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    while not stop["flag"]:
        try:
            ...
            log.warning("btc5m s9b failed: %s", exc)
        if args.once:
            break
