...
def _layered_direction_reason(candidate: dict, direction_bps: float) -> str | None:
    edge_bps = int(candidate["edge_bps"])
    layer_edge = _env_i("BTC5M_LOW_EDGE_CONFIRM_MAX_BPS", 1000)
    low_edge_min_direction = _env_f("BTC5M_LOW_EDGE_MIN_DIRECTION_BPS", 4.0)
    high_edge_min_direction = _env_f("BTC5M_HIGH_EDGE_MIN_DIRECTION_BPS", 2.0)
    if edge_bps < layer_edge and direction_bps < low_edge_min_direction:
        return "low_edge_weak_direction"
    if edge_bps >= layer_edge and direction_bps < high_edge_min_direction:
        return "high_edge_weak_direction"
    return None
...
def run_once(db: sqlite3.Connection, poly: PolyClient, price: BinancePrice, tg: Notifier | None = None) -> dict:
    ...
    direction_reason = _layered_direction_reason(best, direction)
    ...
    elif direction_reason:
        summary["reason"] = direction_reason
