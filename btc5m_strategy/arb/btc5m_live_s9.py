"""S9 Sydney hold-to-expiry time-boxed runner for Polymarket BTC 5m markets.

S9 = small_sydney clone + time gate (skip UTC 0-8 / Beijing 08:00-16:00).

Core rules:
  1. Copy small_sydney parameters exactly
  2. Only trade UTC 8-24 (Beijing 16:00 - 08:00 next day)
        ...
        if now_ts >= float(row["end_epoch"]) + settle_delay:
            ...
            if _opened_with_live_order(row) and live_enabled and not dry_run:
                ...
                if not _order_response_ok(order_response):
                    logging.getLogger("btc5m-live-s9").warning(
...
def main(argv: list[str] | None = None) -> int:
    ...
    log = logging.getLogger("btc5m-live-s9")
    db = _live_db(_env_s("BTC5M_LIVE_DB_PATH", os.path.expanduser("~/.local/share/arb-engine-s9-sydney/btc5m-s9.sqlite")))
    ...
    while not stop["flag"]:
        try:
            ...
            log.info("btc5m s9 done %s", summary)
        except Exception as exc:
            log.warning("btc5m s9 failed: %s", exc)
        if args.once:
            break
