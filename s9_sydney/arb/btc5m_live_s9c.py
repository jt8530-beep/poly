"""S9C Sydney direction-timebox observer for Polymarket BTC 5m markets.

S9 = small_sydney clone + time gate (skip UTC 0-8 / Beijing 08:00-16:00).

Core rules:
  1. Copy small_sydney parameters exactly
  2. Only trade UTC 8-24
  3. UTC 13-20: only UP allowed
  4. Skip abs(direction_bps) < 2 (Beijing 16:00 - 08:00 next day)
  3. Hold to expiry (settled_binance_proxy)
  4. No stop-loss, no tail-force-exit, no pre-settle-loss-cap, no dynamic-exit
  5. Dry-run only. Live order submission requires explicit double-switch.

Time gate is controlled by env vars:
  BTC5M_TIME_GATE_ENABLED=true  (default true)
  BTC5M_TIME_GATE_START_UTC=8
  BTC5M_TIME_GATE_END_UTC=24
"""
...
def _direction_timebox_allowed(side):
    enabled = _env_b("BTC5M_DIRECTION_TIMEBOX_ENABLED", True)
    if not enabled:
        return True, "dir_timebox_disabled"
    now = datetime.now(timezone.utc)
    hour = now.hour + now.minute / 60.0 + now.second / 3600.0
    if 13 <= hour < 20 and str(side).upper() == "DOWN":
        return False, "dir_timebox_blocked_DOWN"
    return True, "ok"
        ...
        if now_ts >= float(row["end_epoch"]) + settle_delay:
            ...
            if _opened_with_live_order(row) and live_enabled and not dry_run:
                ...
                if not _order_response_ok(order_response):
                    logging.getLogger("btc5m-live-s9c").warning(
...
def run_once(db: sqlite3.Connection, poly: PolyClient, price: BinancePrice, tg: Notifier | None = None) -> dict:
    ...
    dir_time_ok, dir_time_reason = _direction_timebox_allowed(str(best["side"]))
    if not dir_time_ok:
        summary["reason"] = dir_time_reason
    elif int(best["edge_bps"]) < min_edge:
        ...
        if notional < _env_f("BTC5M_MIN_NOTIONAL_USD", 5.0):
            ...
            if tg:
                tg.send_html(
                    "<b>BTC5M S9C Sydney candidate</b>\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    log = logging.getLogger("btc5m-live-s9c")
    db = _live_db(_env_s("BTC5M_LIVE_DB_PATH", os.path.expanduser("~/.local/share/arb-engine-s9c-sydney/btc5m-s9c.sqlite")))
    ...
    while not stop["flag"]:
        try:
            ...
            log.info("btc5m s9c done %s", summary)
        except Exception as exc:
            log.warning("btc5m s9c failed: %s", exc)
        if args.once:
            break
