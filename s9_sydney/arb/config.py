...
def _env_s_or_file(name: str, filepath: str, default: str = "") -> str:
    """Read secret: env var first, fallback to file (avoids /proc exposure)."""
    v = os.getenv(name)
    if v:
        return v
    try:
...
class PolyConfig:
    ...
    polygon_private_key: str = _env_s_or_file("POLYGON_PRIVATE_KEY", os.path.expanduser("~/.s9b/prvkey"), "")
