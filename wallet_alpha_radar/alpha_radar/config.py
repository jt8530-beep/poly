...
and `os.getenv()` would silently fall back to defaults — which is the same
bug that produced the 2026-05-26 PLTR-loss postmortem in lp_market_maker.
"""
...
def _load_dotenv() -> int:
    """
    Read `<package>/../.env` (e.g. `wallet_alpha_radar/.env`) and inject any
    ...
    try:
        ...
        logging.getLogger("war.config").warning(".env load failed: %s", e)
    return n


# CRITICAL: this MUST run before the @dataclass class bodies below are
# evaluated, because dataclass field defaults like `_env_s("WAR_X", "")` are
...
def load() -> Config:
    ...
    if _LOADED_DOTENV_COUNT:
        logging.getLogger("war.config").info(
