...
def classify_and_sort(markets: list[MarketSide]) -> tuple[str, list[MarketSide]]:
    ...
    thrs = [extract_threshold(m.question) for m in markets]
    good = [(t, m) for t, m in zip(thrs, markets) if t is not None]
    if len(good) >= 3 and len({t for t, _ in good}) >= 3:
        good.sort(key=lambda x: x[0])
        for t, m in good: m.order_key = t
        return "threshold", [m for _, m in good]
...
def _top_of_book(book: dict) -> tuple[float | None, float | None, float | None, float | None]:
    ...
    # some API shapes return descending asks; normalise
